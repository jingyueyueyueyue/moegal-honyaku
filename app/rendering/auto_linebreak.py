...
def _resolve_initial_segments(
    ...
    if horizontal:
        # 横排：估算行数 = 高度/宽度 * sqrt(text_len) 的合理值
        # 这假设字体大小大致等于行高
        aspect_ratio = safe_height / safe_width
        estimated_segments = max(1, int(round(aspect_ratio * math.sqrt(text_len))))
    else:
        # 竖排：估算列数
        # 假设每列能放 height/font_size 个字符，总列数 = text_len / chars_per_col
        # 使用气泡的宽高比来估算合理的列数
